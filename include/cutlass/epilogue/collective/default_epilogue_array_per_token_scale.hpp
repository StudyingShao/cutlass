...
class SmemEpilogueArrayPerTokenScale {
 public:
  ...
  static bool can_implement(ProblemShape problem_shapes, Arguments const& args) {
    bool const no_source_or_beta =
        args.ptr_C == nullptr && args.beta == ElementCompute(0);
    bool const valid_output_storage =
        args.ptr_D != nullptr &&
        args.dD != nullptr &&
        args.output_base != nullptr &&
        (reinterpret_cast<std::uintptr_t>(args.output_base) %
         (OutputAlignmentBits / 8)) == 0;
    ...
    if (problem_shapes.is_host_problem_shape_available()) {
      for (int group = 0; group < problem_shapes.groups(); ++group) {
        ...
        host_shapes_match = host_shapes_match &&
            channel_extent == args.output_channel_extent &&
    ...
    if (!no_source_or_beta) {
      CUTLASS_TRACE_HOST(
          "  CAN IMPLEMENT: Compact epilogue does not support source C or beta.\n");
    }
    if (!valid_output_storage) {
      CUTLASS_TRACE_HOST(
          "  CAN IMPLEMENT: Output storage must provide a 16B-aligned base and pointer/stride arrays.\n");
    }
    ...
    if (!host_shapes_match) {
      CUTLASS_TRACE_HOST(
          "  CAN IMPLEMENT: Host problem shapes do not match the compact epilogue channel contract.\n");
    }

    return no_source_or_beta && valid_output_storage && valid_output_shape &&
        valid_output_stride && host_shapes_match;
  }
  ...
  CUTLASS_DEVICE void operator()(
    ...
    CUTLASS_ASSERT(
        (reinterpret_cast<std::uintptr_t>(output) % (OutputAlignmentBits / 8)) == 0);
